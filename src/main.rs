...
use crate::repositories::{get_all_active_user_routes, get_route_state, get_station_name, update_route_state, UserRouteWithDetails};
...
use types::{DateRange, PassengerCount, ScrapeRequest, TimeFilter};
...
impl UserTracker {
    ...
    async fn check_and_notify(&self) -> error::Result<()> {
        ...
        let state_changed = state
            .as_ref()
            .is_none_or(|s| s.last_seen_hash != hash_str);
...
fn calculate_state_hash(schedules: &[types::BusSchedule]) -> u64 {
    ...
    for schedule in schedules {
        ...
        for plan in &schedule.available_plans {
            ...
            if let types::SeatAvailability::Available { remaining_seats } = &plan.availability {
